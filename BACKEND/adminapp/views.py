...
from adminapp.selectors.admin_selector import get_users, get_audit_logs
from adminapp.services.user_service import create_user, change_user_status
from adminapp.services.document_service import send_document
...
class UpdateUserStatusView(APIView):
    ...
    def patch(self, request, user_id: str):
        ser = UpdateUserStatusSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        ...
        try:
            ...
            return fail(str(ex), status=403)
        except Exception as ex:
            return fail("Could not update user status", errors={"detail": str(ex)}, status=400)

        message = "User activated" if user["is_active"] else "User deactivated"
        return ok(message=message, data={"user": user})
